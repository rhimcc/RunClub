...
//    
//    func startListeningForClubEvents(clubId: String) {
//        clubListener = firestore.collection("clubs").document(clubId).addSnapshotListener { [weak self] snapshot, error in
//            guard let self = self, let snapshot = snapshot else {
//                print("Error fetching club updates: \(error?.localizedDescription ?? "Unknown error")")
//                return
//            }
//            
//            if let club = try? snapshot.data(as: Club.self) {
//                print("new event to schedule")
//                self.scheduleNotificationsForNewEvents(club: club)
//            }
//        }
//    }
//    
//    func scheduleNotificationsForNewEvents(club: Club) {
//        for eventId in club.eventIds {
//            firestore.collection("events").document(eventId).getDocument { (document, error) in
//                guard let document = document, document.exists, let event = try? document.data(as: Event.self) else {
//                    print("Failed to fetch event: \(error?.localizedDescription ?? "Unknown error")")
//                    return
//                }
//                
//                if club.memberIds.contains(User.getCurrentUserId()) {
//                    let notificationId = event.id ?? UUID().uuidString
//                    UNUserNotificationCenter.current().getPendingNotificationRequests { requests in
//                        if !requests.contains(where: { $0.identifier == notificationId }) {
//                            self.scheduleEventNotification(event: event, forClub: club)
//                        }
//                    }
//                }
//            }
...
//    }
//    
//    func scheduleEventNotification(event: Event, forClub club: Club) {
//        let content = UNMutableNotificationContent()
//        content.title = "Upcoming Event"
//        content.body = "Your event \(event.name) starts in 1 hour!"
//        content.sound = .default
//        
//        let triggerDate = Calendar.current.date(byAdding: .hour, value: -1, to: event.date) // 1 hour before
//        guard let triggerTimeInterval = triggerDate?.timeIntervalSinceNow, triggerTimeInterval > 0 else {
...
//            } else {
//                print("added notification for: \(triggerTimeInterval), for event: \(event.name)")
//            }
//        }
//    }
//    
